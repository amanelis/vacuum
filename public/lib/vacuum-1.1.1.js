(function(window){
     ...
     */
    Vacuum.prototype.handle_error = function(message, file, line, level) {
     ...
     */
    Vacuum.prototype.logger = function(type, message) {
      if (typeof message === 'undefined') return false;
      if (this.config.debug) console.log('Vacuum['+API_KEY+']['+type+']['+message+']');
    };
     ...
     * @param {string} key value for key to get the cookie.
     * @return {string|boolean} value of cookie found.
     */
      ...
      if (USE_XHR) {
        ...
        _xhr.onreadystatechange = function (e) {
          if (_xhr.readyState === 4) {
              _self.logger('xhr', 'ready');
            if (_xhr.status === 201) {
              _self.logger('transmission', 'success');
            } else {
              _self.logger('transmission', 'failure');
            }
          }
        };
        _xhr.send();
     ...
     */
    Vacuum.prototype.run = function() {
      this.blackbox = this.captureData();
      this.transmit(API_URL, this.blackbox);
    };  
    
    /**
     * Enable the vacuum object to be instantiated outside of window.onerror
     */
    window['Vacuum'] = Vacuum;

    /**
     * Call object methods on error
     */
    window.onerror = function(message, file, line) {
      /**
       * Initialize vacuum on an error
       */
      var _instance = new Vacuum({debug: DEBUG_MODE});
      
      /**
       * Methods to set the error and collect data
       */
      _instance.handle_error(message, file, line, 'window');
      _instance.run();
    };
})(window);